...
def draw__trayFrame( pdfcanvas=None, page_bbox=None, cards=None, trayLabel=None ):
    ...
    for key, val in cards.items():
        ...
        # # -- [2-8] radial line & arcs              -- #
        # elif ( val["shapeType"].lower() == "polylines" ):
        #     r1_, th1_, r2_, th2_ = 2, 3, 4, 5
        #     ang2rad  = np.pi / 180.0
        #     center   = np.array( val["center"]    )
        #     points   = val["points"]
        #     for ik,pt in enumerate(points):
        #         if ( points[0].lower() == "line" ):
        #             if ( points[1].lower() == "rt"   ):
        #                 cos1,sin1  = np.cos( pt[th1_]*ang2rad ), np.sin( pt[th1_]*ang2rad )
        #                 cos2,sin2  = np.cos( pt[th2_]*ang2rad ), np.sin( pt[th2_]*ang2rad )
        #                 xp1 ,yp1   = pt[r1_]*cos1+center[x_], pt[:,r1_]*sin1+center[y_]
        #                 xp2 ,yp2   = pt[r2_]*cos2+center[x_], pt[:,r2_]*sin2+center[y_]
        #                 xn1, yn1   = ( xp1 - page_bbox_lb[x_] )*mm, ( yp1 - page_bbox_lb[y_] )*mm
        #                 xn2, yn2   = ( xp2 - page_bbox_lb[x_] )*mm, ( yp2 - page_bbox_lb[y_] )*mm
