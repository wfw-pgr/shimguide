...
def draw__trayFrame( pdfcanvas=None, page_bbox=None, cards=None ):
    ...
    for key, val in cards.items():
        ...
        if ( val["shapeType"].lower() == "polyline_inxy" ):
            ang2rad  = np.pi / 180.0
        ...
        # -- [2-7] side-parts case                  -- #
        if ( val["shapeType"].lower() == "polyline_side" ):
            ang2rad  = np.pi / 180.0
            center   = np.array( val["center"]     )
...
def make__shimGuideMask( jsonFile=None, databaseFile=None, flagFile1=None, flagFile2=None, \
    ...
    pagesize_actual  = pagesize[:] - 2.0*conf["overlap"]
    ...
    outFileBase  = "{0}{1}_{2}_{3}_".format( outDir, settings["trayName"], action, z_character )
    ...
    merge__pdfFile( nParallel=conf["nParallel"], inpFile=inpFile, outFile=outFile, erase=eraseEachFile )
